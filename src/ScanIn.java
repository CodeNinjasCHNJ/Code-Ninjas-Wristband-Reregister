...
public class ScanIn {
    ...
    /**
     * TODO: Change link to your center's Scan In website
     */
    public static final String SCAN_IN_LINK = "https://dojo.code.ninja/scan-in/cn-nj-cherry-hill";

    public static void main(String[] args) {
        String wristbandID = JOptionPane.showInputDialog("Scan Wristband");
        System.out.println("Wristband scanned: " + wristbandID);
        ...
        try {
            ...
            Thread.sleep(3500);
            type(getSecondaryCode(wristbandID));
    ...
    private static void switchWindows() {
        try {
            getDesktop().browse(URI.create(SCAN_IN_LINK));
        } catch (IOException e) {
            e.printStackTrace();
        }
        mRobot.keyPress(VK_ALT);
        mRobot.keyPress(VK_TAB);
        mRobot.keyRelease(VK_TAB);
        mRobot.keyRelease(VK_ALT);
    }
    ...
    private static void doType(int[] keyCodes, int offset, int length) {
        ...
        mRobot.keyPress(keyCodes[offset]);
        doType(keyCodes, offset + 1, length - 1);
        mRobot.keyRelease(keyCodes[offset]);
    }

    private static String getSecondaryCode(String initialCode) {
        try {
            Scanner scanner = new Scanner(new File("C:\\Users\\coden\\IdeaProjects\\ScanInConverter\\src\\codes.txt"));
            scanner.useDelimiter("-");
            while (scanner.hasNext()) {
                String code = scanner.next();
                if (code.trim().equals(initialCode)) {
                    return scanner.next();
                } else {
                    scanner.nextLine();
                }
